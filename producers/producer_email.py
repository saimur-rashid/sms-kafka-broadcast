...
logger.info("🚀 Email Producer Started - Polling Oracle Database...")
...
while True:
    try:
        with pool.acquire() as conn:
            with conn.cursor() as cursor:
                cursor.execute("""
                    SELECT ID, BANK_ID, MESSAGE, EMAIL, RETRY_COUNT
                ...
                rows = cursor.fetchmany(kafka_cfg.get('email_batch_size', 10000))

                if rows:
                    poll_count += 1
                    logger.info(f"📊 Poll #{poll_count} - Found {len(rows)} pending email messages")

                    for row in rows:
                        msg_id, bank_id, message, email, retry_count = row
                    ...
                    producer.flush()
                    conn.commit()
                    logger.info(f"✅ Committed {len(rows)} email messages | Total produced: {messages_produced}")
        ...
        logger.error(f"Email Producer Error: {e}")
        time.sleep(5)